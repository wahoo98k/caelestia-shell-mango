...
StyledWindow {
    ...
    readonly property bool hasSpecialWorkspace: false // mango has no special workspaces
    ...
    // Upstream uses HyprlandFocusGrab here. mango has no equivalent protocol,
    // so the fork drops the grab and relies on the layer-shell input regions in
    // Regions/mask instead -- clicks outside a panel fall through rather than
    // being reported to us. The consequence is that nothing fires onCleared, so
    // panels do not self-close on click-outside; that is the fork's known
    // behaviour difference, not a regression from this merge.
    //
    // The object survives because dragMaskPadding reads focusGrab.active.
    QtObject {
        id: focusGrab
